...
class ClanCog(commands.Cog):
    ...
    async def clan_stats(self, interaction: discord.Interaction, name: str):
        ...
        e.add_field(name="📈 Gesamt 24h", value=f"**+{fmt(max(0, latest['points']-oldest['points']))}**", inline=True)
    ...
    @app_commands.command(name="contributors",
                          description="⭐ Top Clanwar Contributors (wie Screenshot 5)")
    @app_commands.describe(name="Clan Name")
    async def contributors(self, interaction: discord.Interaction, name: str):
        ...
        async with aiohttp.ClientSession() as s:
            ...
            if not data:
                return await interaction.followup.send(f"❌ Clan **{name}** nicht gefunden!")
            rank   = await ps99.get_clan_rank(s, name)
            battle = await ps99.get_active_clan_battle(s)

        battle_name = "ClanBattle"
        if battle:
            battle_name = battle.get("configName", "ClanBattle")

        # Resolve top-10 member names
        ...
        # Patch display names into data for the embed
        for m in data.get("Contribution", {}).get("Battle", []):
            uid = m.get("UserID")
            if uid and uid in uid_map:
                m["_name"] = uid_map[uid]

        emb = embeds.top_contributors(data, name, rank, battle_name)
        # Override embed field with real names
        battle_sorted = ps99.battle_sorted(data)
        lines = []
        for i, m in enumerate(battle_sorted[:10]):
            uid  = m.get("UserID", "?")
            n    = m.get("_name") or db.cached_name(uid) or f"UserID {uid}"
            pts  = m.get("Points", 0)
        ...
        emb.add_field(name="Current Event",  value=f"**{battle_name}**",             inline=True)
        emb.add_field(name="Current Rank",   value=f"**#{rank}**" if rank else "**?**", inline=True)
        emb.add_field(name="Total Stars",    value=f"**{fmt(total_pts)}** ⭐",        inline=True)
        emb.add_field(name="Kick Cooldown",  value=kick_str,                          inline=True)
        emb.add_field(name="Members",        value=f"**{member_count}/{capacity}**",  inline=True)
    ...
    async def member_stats(self, interaction: discord.Interaction,
        ...
        if not uid:
            return await interaction.followup.send(f"❌ Roblox-User **{roblox_user}** nicht gefunden!")
