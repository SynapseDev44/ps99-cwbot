"""
cogs/clan.py  –  !cb, /clan, /clan_rank, /clan_stats, /top_clans, /contributors, /member_stats
"""
import discord
...
class ClanCog(commands.Cog):
    ...
    # ── shared fetch: clan + rank gleichzeitig ────────────────────────────────
    @staticmethod
    async def _fetch(session, name: str):
    ...
    async def cb(self, ctx: commands.Context, *, name: str = ""):
        if not name:
            return await ctx.send("❌ Nutze: `!cb ZYXE`")
        ...
        try:
            ...
            emb = embeds.clan_board(data, name, rank,
    ...
    # ── /clan ─────────────────────────────────────────────────────────────────
    @app_commands.command(name="clan", description="🏆 Clan-Dashboard")
    ...
    async def clan_rank(self, interaction: discord.Interaction, name: str):
        ...
        try:
            async with aiohttp.ClientSession() as s:
                data, rank = await self._fetch(s, name)
            if not data:
                return await interaction.followup.send(f"❌ **{name}** nicht gefunden!")
    ...
    # ── /clan_stats ───────────────────────────────────────────────────────────
    @app_commands.command(name="clan_stats", description="📊 Punkte-Stats der letzten 24h")
    @app_commands.describe(name="Clan Name")
    async def clan_stats(self, interaction: discord.Interaction, name: str):
        ...
        if not snaps:
            return await interaction.followup.send(
                f"❌ Keine Daten für **{name}**. Zuerst `/clan_track` nutzen!")
        ...
        e = discord.Embed(title=f"📊 Stats  •  {name}",
    ...
    async def top_clans(self, interaction: discord.Interaction):
        ...
        try:
            ...
            lines = [
                ...
                f"{c.get('Members',0)}/{c.get('MemberCapacity',75)}"
                for i, c in enumerate(clans)
            ]
            e = discord.Embed(title="🏆 Top 10 — PS99 Global",
                              description="\n".join(lines), color=0xFFD700)
    ...
    async def contributors(self, interaction: discord.Interaction, name: str):
        ...
        try:
            ...
            blist  = ps99.battle_sorted(data)
            lines = []
            for i, m in enumerate(blist[:10]):
                uid  = m.get("UserID", "?")
                uname = uid_map.get(uid) or db.cached_name(uid) or f"ID {uid}"
                lines.append(f"**{i+1}.** {uname} ⭐ **{fmt(m.get('Points',0))}**")
            kick_cd = data.get("LastKickTimestamp")

            e = discord.Embed(title=f"⭐ Top Contributors  •  {name}", color=0xFFD700)
            e.add_field(name="Event",  value=f"**{bname}**",                          inline=True)
            e.add_field(name="Rank",   value=f"**#{rank}**" if rank else "**?**",     inline=True)
            e.add_field(name="Stars",  value=f"**{fmt(ps99.total_points(data))}** ⭐", inline=True)
            e.add_field(name="Kick Cooldown", value=f"<t:{kick_cd}:R>" if kick_cd else "–", inline=True)
    ...
    async def member_stats(self, interaction: discord.Interaction,
        ...
        try:
            async with aiohttp.ClientSession() as s:
                ...
                if not data:
                    return await interaction.followup.send(f"❌ **{clan}** nicht gefunden!")
                uid = await ps99.roblox_uid(s, roblox_user)
            if not uid:
                return await interaction.followup.send(f"❌ User **{roblox_user}** nicht gefunden!")
